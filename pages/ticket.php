    ...
    if(empty($description_err)){
        ...
        if($stmt = $conn->prepare($sql)){
            ...
            if($stmt->bind_param("iss",$user_id,$user_name,$param_description)) {
                ...
                if($stmt->execute()){
                    $get_email = mysqli_query($conn, "SELECT email FROM `user` WHERE `id` = ".$_SESSION['userId']."");
                    $get_email = $result->fetch_assoc()['email'];

                    mail($get_email, "Service IT - Thanks for the ticket!", "Thank you for sending the ticket!"); 
